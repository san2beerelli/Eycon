    ...
    const actions = [
        ...
        return (
            <div style={{marginLeft: 30, marginTop: 5}}>
                <RaisedButton label="Icon" primary={true} onTouchTap={this.handleOpen} />
                 <Dialog title="Select Icon" actions={actions}  modal={false} open={this.state.open}
                        onRequestClose={this.handleClose}
                        autoScrollBodyContent={true}
                        >
                        <div style={gridStyle}>
                            {faIconsArr.map((item,indx) => {
                                  return <GridItem key={indx} name={item} selectedIcon={selectedIcon} onItemSelect={this.onItemSelectHandler}/>
                                })}
                        </div>
                 </Dialog>
            </div>
